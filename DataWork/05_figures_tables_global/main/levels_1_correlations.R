...
df_long <- df %>%
  ...
  dplyr::mutate(variable_cat = case_when(
    ...
    variable %>% str_detect("cnn_viirs_s2_rgb_") ~ "Daytime Imagery, CNN: RGB",
    variable %>% str_detect("cnn_viirs_s2_ndvi_") ~ "Daytime Imagery, CNN: NDVI",
    variable %>% str_detect("cnn_viirs_s2_bu_") ~ "Daytime Imagery, CNN: BU",
