...
# Define functions -------------------------------------------------------------
load_osm_poi <- function(country_code){
  
  ### A. Define directory
  if(country_code %in% "BD") osm_country_dir <- "bangladesh-210101-free"
...
extract_poi <- function(buffer_m, country_code, survey_df){
  ...
  osm_df <- load_osm_poi(country_code)
...
for(buffer_i in c(5000)){
  for(country_code_i in country_codes_all){
    ...
    if(!file.exists(OUT_PATH) | RE_EXTRACT_IF_EXISTS){
      survey_df_i <- extract_poi(buffer_i, country_code_i, survey_df)
