...
split_files_to_new_dirs <- function(dirs_vec,
  ...
  for(dir_i in dirs_vec){
    ...
    dir.create(file.path(osm_dir, "FinalData", dir_1_i))
    dir.create(file.path(osm_dir, "FinalData", dir_2_i))
...
haiti_domrep_dirs <- file.path(osm_dir, "FinalData") %>%
  list.files() %>%
  str_subset("haiti-and-domrep")

split_files_to_new_dirs(dirs_vec = haiti_domrep_dirs,
