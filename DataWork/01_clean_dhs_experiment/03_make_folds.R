...
# Export -----------------------------------------------------------------------
saveRDS(dhs_all_df_coll_folds, file.path(dhs_exp_dir, "FinalData", "Individual Datasets", "survey_socioeconomic.Rds"))
...
df <- dhs_all_df_coll_folds %>%
  ...
  dplyr::mutate(yr2013_est = (yr2018 + yr2008) / 2)
...
# 
# 
# aa <- dhs_all_df_coll_folds %>%
#   left_join(df, by="uid")
# 
# cor.test(aa$pca_allvars, aa$pca_allvars_new)
# 
# aa %>%
#   ggplot() +
#   geom_point(aes(x = pca_allvars_new, y= pca_allvars))
# 
