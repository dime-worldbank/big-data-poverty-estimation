...
countries <- file.path(dhs_exp_dir, "RawData") %>% list.files()
country_year_dirs <- lapply(countries, function(country_i){
  country_year_dir <- file.path(dhs_exp_dir, "RawData", country_i) %>% list.files(full.names = T)
}) %>% 
  unlist()
...
saveRDS(dhs_all_df, file.path(dhs_exp_dir, "FinalData", "Individual Datasets", 
