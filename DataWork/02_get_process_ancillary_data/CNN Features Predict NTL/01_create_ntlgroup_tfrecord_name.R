...
make_tfrecord_name <- function(dhs_all_df_coll,
  ...
  for(fold_name in unique(dhs_all_df_coll$within_country_fold)){
    ...
    tfrecord_name <- paste0(prefix, "_", fold_name, "_", tfrecord_id, ".tfrecord")
...
write.csv(survey_df_clean, file.path(data_dir, SURVEY_NAME, "FinalData", 
                                     "Individual Datasets", 
                                     "data_for_cnn.csv"),
          row.names = F)


write.csv(survey_df_clean, file.path(gdrive_dir, 
